...
public class FeaturesPerceptron extends Model {
		...
		while(nbEpoch<NombreEpoch) {
			while(nbBatch<trainingImages.size()/batchSize)
				while (nbTrainedImages<batchSize) {
					...
					yRealite=0;
					//met a zero les valeurs du tableau correspondantes aux caracteristiques non souhaitees
					for (int i = 0; i < caracteristiques.length; i++) {
			            if(featuresEnabled[i]==false) {
			            	caracteristiques[i]=0;
			            }
			        }
					//extraction de la valeur attendue(yrealite) dans le nom de l image en passant par getLabel()
	 ...
	 * 
	 * @param image
	 * @return -1 if the image does not represent the number five; returns 1 if it does
	 */
